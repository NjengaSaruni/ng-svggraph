...
export class AppComponent implements AfterViewInit {
  ...
  public ngAfterViewInit(): void {
    const viewBox = this.canvas.nativeElement.viewBox.baseVal;
    ...
    const total = data.reduce((t, item) => {
      return t + item.hrs;
    }, 0);
    ...
    const maxDiff = this.maxDiffer(diffs);
    const yInterval = verticalMidPoint / maxDiff;
  ...
  private addText(coords, text, cssClass?) {
    const elem = document.createElementNS(this.canvas.nativeElement.getAttribute('xmlns'), 'text');
    ...
    this.canvas.nativeElement.appendChild(elem);
  }

  private addLine(coords, stroke = '#ff8000') {
    const elem = document.createElementNS(this.canvas.nativeElement.getAttribute('xmlns'), 'line');
  ...
  private addRect(coords, fill = '#ff8000', stroke = '#ffffff') {
    const elem = document.createElementNS(this.canvas.nativeElement.getAttribute('xmlns'), 'rect');
    elem.setAttribute('x', coords.x);
    elem.setAttribute('y', coords.y);
